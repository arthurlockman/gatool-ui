...
const playoffTiebreakers = {
  ...
  "2025": ["foulPoints", "autoPoints", "endGameBargePoints"],
...
function LayoutsWithNavbar({ selectedEvent, practiceSchedule, qualSchedule, playoffs, teamList, communityUpdates,
  ...
  return (
    <>
      <MainNavigation selectedEvent={selectedEvent} qualSchedule={qualSchedule} playoffs={playoffs}
        ...
        worldHighScores={worldHighScores} allianceSelection={allianceSelection} practiceSchedule={practiceSchedule} />
      <Outlet />
      <BottomNavigation />
    </>
  );
...
function App() {
  ...
  const [practiceFileUploaded, setPracticeFileUploaded] = usePersistentState("cache:practiceFileUploaded", false);
  ...
  const [autoHideSponsors, setAutoHideSponsors] = usePersistentState("cache:autoHideSponsors", null);
  ...
  const [showNotesAnnounce, setShowNotesAnnounce] = usePersistentState("cache:showNotesAnnounce", null);
  ...
  const [hidePracticeSchedule, setHidePracticeSchedule] = usePersistentState("cache:hidePracticeSchedule");
  ...
  const [usePullDownToUpdate, setUsePullDownToUpdate] = usePersistentState("cache:usePullDownToUpdate", false);
 ...
 * */
  ...
  */
  async function getSchedule(loadingEvent) {
    ...
    */
    ...
    if (practiceschedule?.schedule?.length > 0) {
      if (practiceFileUploaded) { setPracticeFileUploaded(false); }
  ...
  async function getTeamList(adHocTeamList) {
    if ((teamListLoading === "" || teamListLoading !== selectedEvent?.value?.name) && (selectedEvent?.value?.code !== "OFFLINE" || adHocTeamList)) {
      ...
      await Promise.all(newTeams).then(function (values) {
        ...
        var champsTeams = [];
        if (selectedEvent?.value?.champLevel !== "" || showDistrictChampsStats || (selectedEvent?.value?.code.includes("OFFLINE") && playoffOnly && champsStyle)) {
      ...
      console.log(`Team List for ${selectedEvent?.value?.name} is loading. Skipping...`);
    }
  }
  ...
  async function getAlliances(allianceTemp) {
    ...
    if (!selectedEvent?.value?.code.includes("PRACTICE") && !selectedEvent?.value?.code.includes("OFFLINE")) {
  ...
  */
  const nextMatch = () => {
    if (!adHocMode) {
      if (
        (practiceSchedule?.schedule?.length === 0 && (qualSchedule?.schedule?.length === 0) && (playoffSchedule?.schedule?.length > 0)) ||
        ((practiceSchedule?.schedule?.length > 0 || practiceSchedule?.schedule?.schedule?.length > 0 || offlinePlayoffSchedule?.schedule?.length > 0 || offlinePlayoffSchedule?.schedule?.schedule?.length > 0) &&
          (currentMatch < ((practiceSchedule?.schedule?.length || practiceSchedule?.schedule?.schedule?.length || 0) + (offlinePlayoffSchedule?.schedule?.length || offlinePlayoffSchedule?.schedule?.schedule?.length || 0))
          )
        )
      ) {
