...
function RanksPage({ selectedEvent, teamList, rankings, rankSort, setRankSort, allianceCount, rankingsOverride, setRankingsOverride, setRankings, allianceSelection, getRanks, setAllianceSelectionArrays, playoffs, districtRankings }) {
    ...
    function handleRankingsOverrideFiles(e) {
        ...
        reader.onload = function (e) {
            ...
            var rankingsNew = utils.sheet_to_json(worksheet, { range: 4 });
                ...
                {(rankings?.ranks.length > 0) && rankingsOverride && !playoffs && allianceSelection &&
                    <>
                        <Container fluid>
                            <Row  >
                                <Col xs={2}></Col>
                                <Col xs={8} className={"leftTable"} style={{ cursor: "pointer", color: "darkblue" }} onClick={clickRemoveRanks}><Alert variant={"warning"}><img style={{ float: "left" }} width="50" src="images/excelicon.png" alt="Excel Logo" /> <input type="file" id="RankingsFiles" onChange={handleRankingsOverrideFiles} className={"hiddenInput"} /> You have loaded an exported Schedule file from your Scorekeeper. The data shown below is from that file, not from rankings synced from FIRST.<br />
