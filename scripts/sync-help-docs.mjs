...
/** Bundled into the app JS so Help works when production hosts only route SPA paths (iframe /help-docs.html would get index.html). */
const bundledModulePath = join(repoRoot, "src", "generated", "helpDocsHtml.js");

function writeBundledHelpModule(html) {
  mkdirSync(join(repoRoot, "src", "generated"), { recursive: true });
  const moduleSource = `/* eslint-disable max-len */
// AUTO-GENERATED by scripts/sync-help-docs.mjs — do not edit.

const helpDocsHtml = ${JSON.stringify(html)};
export default helpDocsHtml;
`;
  writeFileSync(bundledModulePath, moduleSource, "utf8");
}
...
function buildFromWiki() {
  ...
  try {
    ...
    writeBundledHelpModule(written);
    console.log(`Help docs synced from wiki (${pages.length} pages).`);
    console.log(
      "Ship this snapshot: git add public/help-docs.html public/help-wiki-manifest.json src/generated/helpDocsHtml.js && git commit -m \"Update help from wiki\""
    );
  } finally {
...
function main() {
  try {
    ...
    if (existsSync(outputPath)) {
      ...
      writeBundledHelpModule(existing);
